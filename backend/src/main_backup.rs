...
async fn main() -> std::io::Result<()> {
    ...
    log::info!("🔧 Agent engine created, about to start automation scheduler");
    // Start automation scheduler
    let pool_automation = pool.clone();
    let agent_engine_automation = agent_engine.clone();
    log::info!("🔧 Spawning automation scheduler task...");
    tokio::spawn(async move {
    ...
    log::info!("✅ Automation scheduler task spawned");
